...
function generaArray (min, max) {
  ...
  var i = min;
  while (i <= max) {
    var numeroRandomBomb = generaNumeroIntero(1, difficoltaScelta);
...
function userNumbers(difficulty, bomb) {
  var arrayNumeriUtente = [];
  var controlloNumeroUtente = false;
  var i = 0;
  while (controlloNumeroUtente === false && i < difficulty) {
    var numeroUtente = parseInt(prompt('Hai scelto la difficoltà con ' + difficulty + ' numeri. Inserisci un numero da 1 a ' + difficoltaScelta + '.Rimangono ' + ((difficulty - bomb.length) - i) + ' numeri'));
